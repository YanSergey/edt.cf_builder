...
## Добавляет в 1C:EDT возможности:
        ...
        - Сравнение/объединение существующего проекта с cf/cfe-файлом
        - Экспорт проекта в cf/cfe-файл (сборка cf/cfe-файла)

## Возможные сценарии работы:
1. Импорт CF/CFE:

        1.1. Импорт в новый проект через временную инфобазу (должны быть доступны локальные лицензии 1С)
        1.2. Импорт в новый проект через существующую инфобазу (база будет привязана к проекту)
        1.3. Импорт в существующий проект через существующую привязанную инфобазу
        1.4. Импорт в существующий проект через существующую не привязанную к проекту инфобазу (база будет привязана к проекту)

        Опции: 
                - отключить поддержку, т.е. снять замки (только для CF)

2. Экспорт CF/CFE:

        2.1. Экспорт через временную инфобазу
        2.2. Экспорт через существующую привязанную базу
        2.3. Экспорт через существующую не привязанную базу (база будет привязана к проекту)

        Опции:
                - создать CF-поставки (только для CF)
                - загрузить полностью
